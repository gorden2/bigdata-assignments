...
public class BuildInvertedIndexCompressed extends Configured implements Tool {
  ...
  private static class MyReducer extends
      Reducer<PairOfStringInt, IntWritable, Text, ArrayListWritable<VIntWritable>> {
    ...
    public void reduce(PairOfStringInt key, Iterable<IntWritable> values, Context context)
	  ...
	  if (key.getRightElement()== -1){
	       ...
	       postings.add(new VIntWritable(sum));
	       } else {
		  did = key.getRightElement() - did;
		  postings.add(new VIntWritable(did));
		  did = key.getRightElement();
		  // TF
		  postings.add(new VIntWritable(sum));
